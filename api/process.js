        ...
        for (const part of attachParts) {
          ...
          if (analysis && analysis.makerName && analysis.amount > 0 && analysis.type !== 'none') {
            ...
            if (analysis.items && analysis.items.length > 0) {
              await appendItemsToSheet(token, targetSheet, analysis.makerName, analysis.items, yearMonth, dateStr);
            }
 ...
 */
async function appendItemsToSheet(token, sheetId, makerName, items, yearMonth, dateStr) {
  try {
    // 「明細」シートがあるか確認、なければ作成
    ...
    if (!sheets.includes(detailSheet)) {
      ...
      await googleApi(token,
        ...
        { method: 'PUT', body: JSON.stringify({ values: [['日付', '取引先', '商品名', '金額', '年月']] }) }
      );
    }

    // 商品名を行として追加
    const rows = items.map(item => [dateStr, makerName, item, '', yearMonth]);
    await googleApi(token,
      `https://sheets.googleapis.com/v4/spreadsheets/${sheetId}/values/${encodeURIComponent(detailSheet)}!A:E:append?valueInputOption=USER_ENTERED`,
